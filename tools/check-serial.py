...
with open(out/'serial-qemu.log','wb') as log:
    ...
    try:
        ...
        s.sendall(b"DISPLAY=:0 /usr/share/felix/check-motion; echo MOTION_RESULT:$?; DISPLAY=:0 /usr/share/felix/check-fonts; echo FONT_RESULT:$?; DISPLAY=:0 felix-apps notepad >/tmp/notepad-font.log 2>&1 & app=$!; DISPLAY=:0 felix-about >/tmp/about-font.log 2>&1 & about=$!; DISPLAY=:0 felix-apps wifi --hardware >/tmp/hardware-ui.log 2>&1 & hw=$!; sleep 2; kill -0 $app && kill -0 $about && kill -0 $hw; echo FONT_APPS_RESULT:$?; cat /tmp/notepad-font.log /tmp/about-font.log; kill $app $about $hw; printf '\\nFONT_CHECK_DONE\\n'\n")
        ...
        assert b'\r\nMOTION_RESULT:0\r\n' in data, 'Window animation regression failed'

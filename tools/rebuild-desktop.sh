...
cp "$root/userland/apps/wifi-hardware.h" "$work/src/apps/"
...
chroot "$work" apk add libpng-dev imlib2-dev libxrandr-dev libxft-dev
...
chroot "$work" sh -c 'cc -Os /src/src/felix-root.c -lpng -lX11 -o /out/felix-root; cc -Os /src/src/felix-about.c $(pkg-config --cflags --libs xft) -lX11 -o /out/felix-about; strip /out/felix-root /out/felix-about'
